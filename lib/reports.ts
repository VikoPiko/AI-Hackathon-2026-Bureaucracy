...
export function downloadProcedureChecklist(
  ...
  const checklist = [
    ...
    "",
    "Legal basis:",
    ...((response.legalBasis || []).map((item) => `- ${item}`)),
    "",
    "Scope:",
    ...((response.covers || []).map((item) => `- Covers: ${item}`)),
    ...((response.notCovered || []).map((item) => `- Not covered: ${item}`)),
    "",
    "Eligibility and prerequisites:",
    ...((response.eligibility || []).map((item) => `- Eligible: ${item}`)),
    ...((response.prerequisites || []).map((item) => `- Prerequisite: ${item}`)),
    ...((response.exceptions || []).map((item) => `- Exception: ${item}`)),
    "",
    "Timeline details:",
    ...((response.timelineDetails || []).map((item) => `- ${item}`)),
    "",
    "Cost breakdown:",
    ...((response.costBreakdown || []).map((item) => `- ${item}`)),
    "",
    "Potential risks:",
    ...((response.risks || []).map((item) => `- ${item}`)),
    "",
    "Positive points:",
    ...((response.positivePoints || []).map((item) => `- ${item}`)),
    "",
    "Missing confirmations:",
    ...((response.missingClauses || []).map((item) => `- ${item}`)),
    "",
    "Common mistakes:",
    ...((response.commonMistakes || []).map((item) => `- ${item}`)),
    "",
    "Scams to avoid:",
    ...((response.scamsToAvoid || []).map((item) => `- ${item}`)),
    "",
    "What not to do:",
    ...((response.whatNotToDo || []).map((item) => `- ${item}`)),
    "",
    "Official source:",
...
export function downloadProcedureReport(
  ...
  const report = [
    ...
    "",
    "Related procedures:",
    ...((response.relatedProcedures || []).map((item) => `- ${item}`)),
    "",
